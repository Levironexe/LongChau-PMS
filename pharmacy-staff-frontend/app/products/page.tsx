    ...
    if (editingProduct) {
      ...
      if (editingProduct.product_type === 'medicine') {
        const medicineData = {
          ...
          ...data,
          is_prescription: data.requires_prescription,
        }
        updateMedicine.mutate(medicineData, {
          onSuccess: () => handleSuccess("Medicine updated successfully!"),
          onError: handleError,
        })
        ...
        updateSupplement.mutate(supplementData, {
          onSuccess: () => handleSuccess("Supplement updated successfully!"),
          onError: handleError,
        })
        ...
        updateMedicalDevice.mutate(deviceData, {
          onSuccess: () => handleSuccess("Medical device updated successfully!"),
          onError: handleError,
        })
      }
    } else {
      ...
      if (data.product_type === 'medicine') {
        ...
        if (useFactoryPattern && canCreateMedicineViaFactory) {
          createMedicineFactory.mutate(medicineData, {
            onSuccess: () => handleSuccess("Medicine created successfully via Factory Pattern!"),
            onError: handleError,
          })
        } else {
          createMedicine.mutate(medicineData, {
            onSuccess: () => handleSuccess("Medicine created successfully!"),
            onError: handleError,
          })
        }
      } else if (data.product_type === 'supplement') {
        const supplementData = data
        
        // Use factory pattern if enabled and available
        if (useFactoryPattern && canCreateSupplementViaFactory) {
          createSupplementFactory.mutate(supplementData, {
            onSuccess: () => handleSuccess("Supplement created successfully via Factory Pattern!"),
            onError: handleError,
          })
        } else {
          createSupplement.mutate(supplementData, {
            onSuccess: () => handleSuccess("Supplement created successfully!"),
            onError: handleError,
          })
        }
      } else if (data.product_type === 'medical_device') {
        const deviceData = data
        createMedicalDevice.mutate(deviceData, {
          onSuccess: () => handleSuccess("Medical device created successfully!"),
          onError: handleError,
        })
      }
    }
  ...
  const handleDelete = (product: Product) => {
    if (product.product_type === 'medicine') {
      deleteMedicine.mutate(product.id, {
        onSuccess: () => handleSuccess("Medicine deleted successfully!"),
        onError: handleError,
      })
    } else if (product.product_type === 'supplement') {
      deleteSupplement.mutate(product.id, {
        onSuccess: () => handleSuccess("Supplement deleted successfully!"),
        onError: handleError,
      })
    } else if (product.product_type === 'medical_device') {
      deleteMedicalDevice.mutate(product.id, {
        onSuccess: () => handleSuccess("Medical device deleted successfully!"),
        onError: handleError,
      })
    }
  }
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-2xl">
          ...
          <ProductForm
            ...
            canCreateMedicineViaFactory={canCreateMedicineViaFactory}
            canCreateSupplementViaFactory={canCreateSupplementViaFactory}
